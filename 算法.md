...
ListNode* sortList(ListNode* head) {
        ...
        return mergeTwoLists(left, right);	//合并链表
    }
```
...
创建每一行，按字符串顺序向每行中添加字符，值的注意的是如何让`r`上下摆动
...
构建一个单调递减栈，栈中存储温度id，遍历温度。

- 若当前温度低于栈顶，则入栈

- 否则循环出栈（此时记录`ans[index] = i - index`），直到栈空/栈顶温度高于当前温度（恢复为单调栈），将当前温度入栈
...
使用一个递增的优先队列，维持其长度为k，则队列头就是第k大大元素
...
思路是深度优先搜索+记忆化，我们先找到所有0元素，从0元素开始广度优先搜索，其相邻非零元素距离0的距离一定是1
...
vector<vector<int>> updateMatrix(vector<vector<int>>& mat) {
    int il = mat.size();
    int jl = mat[0].size();
  	//0元素本身离0点距离为0，于是初始将结果全置零
    vector<vector<int>> ans(il, vector<int>(jl, 0));
  	//若used[i][j] == 1，说明这个位置被搜索过了
    vector<vector<int>> used(il, vector<int>(jl, 0));
  	//找到所有0元素
    queue<pair<int,int>> q;
    for(int i = 0; i < il; ++i){
        for(int j = 0; j < jl; ++j){
    ...
    while(!q.empty()){
        ...
        for(int k = 0; k < 4; ++k){
            int ni = i + dirs[k][0];
            int nj = j + dirs[k][1];
            if(0 <= ni && ni < il && 0 <= nj && nj < jl && used[ni][nj] == 0){
                ans[ni][nj] = ans[i][j] +1;
                used[ni][nj] = 1;
                q.push({ni, nj});
            }
